// Script to place sell orders on ROFLSwapOracle
...
async function main() {
  colorLog("\n=== PLACING SELL ORDER ON ROFLSWAPORACLE ===\n", COLORS.BLUE);
  ...
  try {
    ...
    const price = ethers.parseEther("0.9");   // Price per token (0.9 FIRE per WATER - slightly lower than buy price)
    const size = ethers.parseEther("5");     // Amount to sell (5 WATER tokens)
    ...
    colorLog("\nPlacing sell order on ROFLSwapOracle...", COLORS.YELLOW);
    ...
    if (orderPlacedEvent) {
      ...
      try {
        ...
        if (isFilled) {
          ...
          colorLog("The oracle should match it when a compatible buy order is available.", COLORS.YELLOW);
        }
      } catch (error) {

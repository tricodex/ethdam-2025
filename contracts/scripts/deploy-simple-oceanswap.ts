...
async function main() {
  console.log("Deploying SimpleOceanSwap to network:", hre.network.name);
  ...
  try {
    // Deploy SimpleOceanSwap
    console.log("Deploying SimpleOceanSwap...");
    console.log("Constructor params:", [WATER_TOKEN_ADDRESS, FIRE_TOKEN_ADDRESS]);
    
    const simpleOceanSwapFactory = await hre.viem.deployContract("SimpleOceanSwap", [
      WATER_TOKEN_ADDRESS,
      FIRE_TOKEN_ADDRESS
    ]);
    
    const simpleOceanSwapAddress = await simpleOceanSwapFactory.address;
    console.log("SimpleOceanSwap deployed to:", simpleOceanSwapAddress);
    
    // Get contract instance
    const simpleOceanSwap = await hre.viem.getContractAt("SimpleOceanSwap", simpleOceanSwapAddress);
    
    // Set the ROFL app ID
    console.log("\nSetting ROFL app ID in SimpleOceanSwap contract...");
    ...
    try {
      ...
      const setRoflAppTx = await simpleOceanSwap.write.setRoflApp([roflEthAddress]);
    ...
    console.log("\nTesting basic interaction with SimpleOceanSwap...");
    
    const tokenAddresses = await simpleOceanSwap.read.getTokens();
    console.log(`Water Token: ${tokenAddresses[0]}`);
    console.log(`Fire Token: ${tokenAddresses[1]}`);
    
    const roflAppAddress = await simpleOceanSwap.read.roflApp();
    ...
    const deploymentInfo = {
      ...
      simpleOceanSwap: simpleOceanSwapAddress,
      roflAppId: ROFL_APP_ID,
      deploymentTime: new Date().toISOString()
    };
    
    fs.writeFileSync(
      `simple-oceanswap-deployment-${hre.network.name}.json`, 
      JSON.stringify(deploymentInfo, null, 2)
    );
    
    console.log(`\nDeployment information saved to simple-oceanswap-deployment-${hre.network.name}.json`);
    ...
    if (isSapphireNetwork) {
      if (hre.network.name === "sapphire-testnet") {
        ...
        console.log(`https://testnet.explorer.oasis.io/address/${simpleOceanSwapAddress}`);
      } else if (hre.network.name === "sapphire") {
        console.log("You can view your deployment on the Sapphire Mainnet Explorer at:");
        console.log(`https://explorer.oasis.io/address/${simpleOceanSwapAddress}`);
      }
    }
    
    console.log("\n🌊 SimpleOceanSwap Deployment Completed Successfully! 🌊");

...
// Deployed token addresses - reuse the same tokens
...
async function main() {
  console.log("Deploying ROFLSwapV2 to network:", hre.network.name);
  ...
  try {
    // Deploy ROFLSwapV2
    console.log("Deploying ROFLSwapV2...");
    console.log("Constructor params:", [WATER_TOKEN_ADDRESS, FIRE_TOKEN_ADDRESS]);
    
    const roflSwapFactory = await hre.viem.deployContract("ROFLSwapV2", [
      WATER_TOKEN_ADDRESS,
      FIRE_TOKEN_ADDRESS
    ]);
    
    const roflSwapAddress = await roflSwapFactory.address;
    console.log("ROFLSwapV2 deployed to:", roflSwapAddress);
    
    // Get contract instance
    const roflSwap = await hre.viem.getContractAt("ROFLSwapV2", roflSwapAddress);
    
    // Set the ROFL app ID
    console.log("\nSetting ROFL app ID in ROFLSwapV2 contract...");
    ...
    console.log("\nTesting basic interaction with ROFLSwapV2...");
    ...
    const deploymentInfo = {
      ...
      roflSwapV2: roflSwapAddress,
      roflAppId: ROFL_APP_ID,
      deploymentTime: new Date().toISOString()
    };
    
    fs.writeFileSync(
      `roflswap-v2-deployment-${hre.network.name}.json`, 
      JSON.stringify(deploymentInfo, null, 2)
    );
    
    console.log(`\nDeployment information saved to roflswap-v2-deployment-${hre.network.name}.json`);
    ...
    console.log("\n🌊 ROFLSwapV2 Deployment Completed Successfully! 🌊");
    console.log("This improved version has better order tracking and additional helper functions.");

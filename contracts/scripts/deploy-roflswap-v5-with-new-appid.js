// Deploy ROFLSwapV5 contract with the new ROFL app ID
const hre = require("hardhat");
const fs = require("fs");

async function main() {
  console.log("Starting deployment of ROFLSwapV5 with new ROFL app ID...");
  ...
  // Set the new ROFL App ID
  const newRoflAppId = "rofl1qzd2jxyr5lujtkdnkpf9xuh8dktu73nl5q7cp972";
  
  console.log(`Using new ROFL App ID: ${newRoflAppId}`);
  console.log(`ROFL App ID length: ${newRoflAppId.length} characters`);
  ...
  try {
    ...
    const { ethers } = hre;
    const roflAppIdBytes = ethers.toUtf8Bytes(newRoflAppId);
    ...
    console.log("- ROFL App ID:", newRoflAppId);
